...
plot width height region =  intercalate "\n" [makeRow y | y <- [height, height-1..(-height)]]
  where
    makeRow y = [if region (fromIntegral x, fromIntegral y) then '*' else '.' | x <- [-width..width]]
...
bfs start expand = bfsHelper S.empty [(start, 0)]
  where
    bfsHelper visited [] = []
    bfsHelper visited ((s, d):ss) =
      if S.member s visited then
        bfsHelper visited ss
      else
        (s, d) : bfsHelper (S.insert s visited) (ss ++ [(n, d + 1) | n <- expand s])

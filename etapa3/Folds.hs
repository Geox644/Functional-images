    ...
    +
      ...
      *
        +(1.0,2.0)
          Circle 3.0
        Rectangle 4.0 5.0
-- fiecare nivel de copii este indentat cu două spații față de nivelul părintelui

-} 
-- genereaza sirul de caractere dupa numarul specificat de spatii
indent :: Int -> String
indent n = replicate (2 * n) ' '

instance Show RegionAST where
    show x = foldRegionAST helper x 0
      where
        helper :: RegionCombiner (Int->String)
        helper (FromPoints points) = \acc -> indent acc ++ "FromPoints " ++ show points
        helper (Rectangle width height) = \acc -> indent acc ++ "Rectangle " ++ show width ++ " " ++ show height
        helper (Circle radius) = \acc -> indent acc ++ "Circle " ++ show radius
        helper (Complement region) = \acc -> indent acc ++ "~\n" ++ region (acc + 1)
        helper (Union region1 region2) = \acc -> indent acc ++ "+\n" ++ region1 (acc + 1) ++ "\n" ++ region2 (acc + 1) 
        helper (Intersection region1 region2) = \acc -> indent acc ++ "*\n" ++ region1 (acc + 1) ++ "\n" ++ region2 (acc + 1)
        helper (Transform transformation region) = \acc -> indent acc ++ show transformation ++ "\n" ++ region (acc + 1)
{-
    *** TODO ***
    ...
    +
      ...
      *
        +(1.0,2.0)
          Circle 3.0
        Rectangle 4.0 5.0
--  transformă un întreg într-o regiune care include doar punctul dat de perechea (n, n).
-}
...
instance Functor TransformationShape where
    ...
    fmap f (Combine a) = Combine (fmap f a)
...
{-
    *** TODO ***
    ...
    Functor permite aplicarea unei funcții asupra valorilor dintr-un RegionShape
    și obținerea unei noi valori de același tip RegionShape
-}
...
{-
    *** TODO ***
    ...
    Scopul funcției este să parcurgă recursiv structura transformation și să aplice funcția combiner
     asupra fiecărui element al acesteia
-}
...
{-
    ...
    * Implementarea ar trebui să fie aproape identică cu cea din etapa 2,
      ...
      din etapa 2.
    - convertește un obiect de tip TransformationAST într-un obiect de tip Transformation.
-}
...
{-
    *** TODO ***
    ...
    Răspuns: evaluarea lenesa se aplica doar pe componenta ce se doreste a fi aflata
...
basicEntityCount = foldRegionAST combiner
  where
    ...
    combiner (Complement (regCount, transCount)) = (regCount, transCount)
    combiner (Union (regCount1, transCount1) (regCount2, transCount2)) = 
        (regCount1 + regCount2, transCount1 + transCount2)
    combiner (Intersection (regCount1, transCount1) (regCount2, transCount2)) = 
        (regCount1 + regCount2, transCount1 + transCount2)
    combiner (Transform t (regCount, transCount)) = 
        (regCount, transCount + basicTransformationCount t)

...
{-
    *** TODO ***
    ...
    - un operator de aplicare a funcțiilor care are o precedență foarte mică,
    ceea ce înseamnă că tot ce vine după el este evaluat ca un singur argument
    ...
    > inside (1, 1) (== (0, 0))
    False
    $ aplica o functie la un argument.
-}
...
{-
    *** TODO ***
    ...
    liste de puncte.
    -- verifica daca un elementul se afla in lista de puncte
    ...
    False
    verifica dacă un anumit punct se află în lista de puncte dată
-}
...
{-
   ...
   -> Punctul (x, y) va fi în interiorul dreptunghiului dacă și numai dacă:

    -width/2 <= x <= width/2
    -height/2 <= y <= height/2

    Aceasta funcție primește un punct (x, y) și verifică dacă
    se află în interiorul dreptunghiului definit de lățimea și înălțimea date.
...
{-
    *** TODO ***
    ...
    -> verifica dacă coordonatele unui punct (x, y) se află în interiorul cercului definit de raza dată.
    -> Un punct (x, y) se află în interiorul cercului dacă și numai dacă distanța sa față de origine este
     mai mică sau egală cu raza cercului
...
-} -- formula distantei 
circle :: Float -> Region
circle radius = \(x, y) -> sqrt (x^2 + y^2) <= radius
...
{-
    ...
    * intercalate pentru alipirea mai multor liste folosind un element
    ...
    - creează o listă de șiruri de caractere, fiecare corespunzând unei linii
    pe suprafața de desenare. Folosim reverse pentru a începe de la linia de
    sus, până la linia de jos. Pentru fiecare valoare de y,
    row generează un șir de caractere care reprezintă linia corespunzătoare,
    parcurgând valorile de x de la -width la width.
...
plot width height region =  intercalate "\n" [row y | y <- reverse [-height .. height]]
  where
    row y = [plotCh x y | x <- [-width..width]]
    plotCh x y
      | region (fromIntegral x, fromIntegral y) = '*'
      | otherwise = '.'    
...
{-
    *** TODO ***
    ...
    5.0
    -> returnează o funcție care ia un Point și returnează un c prin aplicarea funcțiilor g1 și g2 pe point, apoi aplicând f pe rezultatele obținute.
-}
...
{-
    *** TODO ***
    ...
    ..*..
    .....
    - dacă un punct transformat aparține regiunii, mai întâi aplicăm transformarea asupra punctului
    și apoi verificăm apartenența punctului transformat la regiunea originală.
-}
...
{-
    *** TODO ***
    ...
    Răspuns: Evaluarea leneșă permite calcularea valorilor doar atunci când sunt necesare.
    Acest lucru este util atunci când dorim să accesăm doar o componentă a unei perechi,
    deoarece ne permite să calculăm doar valoarea acelei componente,
    fără a fi nevoie să calculăm și cealaltă componentă.
    funcția circles construiește o regiune formată dintr-un cerc central și un număr de cercuri mai mici,
    plasate în dreapta cercului central, într-o linie orizontală. Numărul total de cercuri este determinat de valoarea lui n.
   - construiește o regiune formată din n cercuri de rază 2, translatate succesiv cu 6 unități pe orizontală.
-}
circles :: Int -> Region
circles n
    | n <= 0    = const False
...
{-
    *** TODO BONUS ***
    ...
    - daca starea nu a fost vizitata, o adaug la rezultat
    - adaug starea la vizitate si extind coada cu noile stari generate de functia expand,
    - marind distanta cu 1 pentru fiecare stare
-}

-- bfsAux este o functie recursiva pentru coada de stari si distante
bfs :: (Ord a) => a -> (a -> [a]) -> [(a, Int)]
bfs start expand = bfsAux S.empty [(start, 0)]
  where
    bfsAux visited [] = []
    bfsAux visited ((s, d):queue_tail) =
      -- verific daca starea actuala a fost vizitata
      if S.member s visited then
        bfsAux visited queue_tail
      else
        (s, d) : bfsAux (S.insert s visited) (queue_tail ++ [(n, d + 1) | n <- expand s])
...
regionAvoidingBfs start region = bfs start expand
  where
    expand (x, y) = filter (not . region) [(x+1, y), (x-1, y), (x, y+1), (x, y-1)]
